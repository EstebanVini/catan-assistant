...
- **ExtraRules** (toggles del host en el lobby): `unequalTrades, sharedPorts, noSpecialBuild, robberNoStealFirstRound, robberEmptyGivesResource, unlimitedProgressCards` (C&K: sin límite de 4 cartas de progreso). El toggle de reparto inicial vive aquí en la UI como **"No repartir recursos de inicio"** (default off; `seedInitialResources` sigue `true` por defecto).
...
- **Mercancías** (`coin`/`paper`/`cloth`): segundo tipo de carta; SOLO las producen las ciudades sobre montaña/bosque/pastura (1 recurso + 1 mercancía en vez de 2 recursos), también en el **reparto inicial** de la ciudad de salida. Banco de mercancías informativo (12 c/u). **Se comercian** entre jugadores y con banco/puertos (recurso↔mercancía, mercancía↔mercancía); ratios en `bankTradeRatioCK`.
- **Mejoras de ciudad** en 3 disciplinas (`trade`/`politics`/`science`), niveles 0–5, se pagan con la mercancía de la disciplina. **Nivel 3 desbloquea habilidad FUNCIONAL**: Guilda/Casa de comercio (Comercio → 2:1 de mercancías en el banco), Fortaleza (Política → promover caballeros a poderosos) y Acueducto (Ciencia → si no produces al tirar, tomas 1 recurso del banco vía `aqueduct:pick`). Nivel 4 reclama **metrópolis** (+2 PV), nivel 5 la blinda.
- **Cartas de progreso** (3 mazos de 18, límite de mano 4 salvo regla `unlimitedProgressCards`): reemplazan a las de desarrollo, se reparten por el **calendario de la ciudad** (dado de evento de color + dado rojo ≤ nivel). El servidor **automatiza 21 de 25** (efectos en `progress:play`); solo siguen "en mesa" **4** por geometría física de tablero o tirada manual: `alchemist` (fija los dados antes de tirar), `inventor` (intercambia fichas de número), `diplomat` (quita el tramo final de un camino) e `intrigue` (expulsa un caballero de una encrucijada). El **Obispo** (`bishop`) mueve el ladrón en "modo Obispo" (`pendingBishop`) y roba 1 carta a **cada** dueño del hex; **Saboteador** (`saboteur`), **Boda** (`wedding`) y **Puerto de mercancías** (`commercialHarbor`) se resuelven al azar como el resto de robos automáticos. Flags de turno: `merchantFleet`, `craneDiscount`, `freeRoads`. El **comerciante** (`state.merchant {ownerId, resource}`) da 2:1 de su recurso y +1 PV (`playerVP`). **No se acaban:** las cartas que salen de la mano (jugadas no-VP y descartadas por límite) caen a `progressDiscards` y se **reciclan** (rebarajado al mazo vacío vía `drawProgressCard`).
...
- Eventos socket nuevos: `lobby:setCitiesKnights`, `turn:rollCK`, `progress:discard/play` (payload con `resource/commodity/targetPlayerId/knightIds/settlementId`), `city:upgrade/buildWall`, `aqueduct:pick`, `knight:build/activate/promote/action`, `barbarian:downgradeCity`. `trade:bank` admite `giveKind/receiveKind` (recurso/mercancía) y `trade:offer` lleva `giveCommodities/receiveCommodities`.
...
- **Pantallas:** `LoginScreen` (registro/login/invitado), `HomeScreen` (crear/unirse/reconectar + card "Cómo jugar"), `LobbyScreen` (código, colores, orden de turnos, bank manager, registro de construcciones iniciales, toggles de reglas), `GameScreen` (la principal), `ProfileScreen` (avatar, displayName, color, **stats + XP/logros**), `InstructionsScreen` (**"Cómo jugar"**: instructivo in-app en español; contenido en `screens/instructionsContent.ts`, imágenes en `public/instructions/`). Navegación por estado: `homeView: 'home' | 'profile' | 'instructions'` (App.tsx; `'instructions'` NO exige cuenta).
...
- **Componentes Caballeros y Ciudades:** `DiceInputCK` (3 dados), `KnightsPanel`, `BarbarianTrack`, `BarbarianLossModal`, `CityCalendarPanel`, `WallControl`, `ProgressHand`, `DevCardPreview`, `CommodityMonopolyPickerModal`, `ResourceMonopolyPickerModal`, `RoadBuildingConfirmModal`, `ContextBanner`, `TopBar`. **Pickers para jugar cartas de progreso** (enrutados por `ProgressHand`): `MerchantPlacementPickerModal` (Mercader), `MerchantFleetPickerModal` (Flota Mercante: recurso o mercancía), `OpponentTargetPickerModal` (Espía/Maestro Mercader/Desertor + sub-paso de caballero), `SettlementUpgradePickerModal` (Medicina), `KnightPromotePickerModal` (Fragua), y `AqueductPickModal` (Acueducto, montado en `GameScreen`). `TradeModal`/`TradeIncomingModal` manejan mercancías; `PublicPlayersPanel` muestra la insignia del comerciante. Íconos C&K (mercancías, caballeros, disciplinas) en `assets/icons.tsx`, varios reciclados (ver `missing-icons.md`). Hay un `FireGlyph` ya disponible en `icons.tsx`.
...
- **Cliente→Servidor (handlers.ts):**
  ...
  - *Caballeros y Ciudades:* `progress:discard/play`, `city:upgrade/buildWall`, `aqueduct:pick`, `knight:build/activate/promote/action`, `barbarian:downgradeCity`. (`trade:bank` admite mercancías vía `giveKind/receiveKind`; `trade:offer` lleva `giveCommodities/receiveCommodities`.)
...
- `cambios-CC-plan-5.md` — 5ª tanda (8 bugs C&K): ícono de Ciencia (símbolo interior), Grúa en el calendario, Obispo/Saboteador/Boda/Puerto de mercancías automáticos, Acueducto en el 7, tratos vacíos/desiguales. Con tareas por agente. Rama: `main`.
...
**Entregado (`docs/cambios-CC-plan.md`, junio 2026, rama `Fix]caballeros-Ciudades`):** la tanda de bugs de Caballeros y Ciudades de `cambios.txt`, con commit por bug verificado (81 tests de servidor + build de cliente en verde):
- **(1) Cartas de progreso:** el servidor automatiza ~17 de 25 (antes 7): además de las VP/monopolios/ingeniero/irrigación/minería, ahora **Estratega** (activa caballeros), **Grúa** (descuento de 1 mercancía en la próxima mejora), **Construcción de carreteras** (2 caminos gratis vía `freeRoads`), **Flota Mercante** (2:1 de un tipo el turno), **Espía** (roba 1 carta de progreso al azar), **Maestro Mercader** (roba 2 cartas a un rival con más PV), **Medicina** (poblado→ciudad por 2 mineral+1 trigo), **Fragua** (promueve hasta 2 caballeros), **Desertor** (un rival pierde un caballero y tú colocas uno igual) y **Mercader** (ver abajo). Siguen "en mesa" solo las de geometría/elección física multi-jugador: alchemist, inventor, diplomat, intrigue, saboteur, wedding, commercialHarbor, bishop. Conteos de mazo ya coincidían con `docs/Progress-Cards-CC.md` (18/18/18); las dev cards base no existen en C&K (no se compran, panel oculto). UI: `ProgressHand` enruta cada carta a su picker.
- **(3) Mercancías iniciales:** la ciudad de salida reparte 1 mercancía por ficha de ore/lumber/wool (`applyInitialSetup` + `commodityGrants`).
- **(6) Habilidades nivel 3 FUNCIONALES:** **Guilda** (Comercio 3 → 2:1 de mercancías en el banco), **Fortaleza** (Política 3 → promover a poderoso; ya existía) y **Acueducto** (Ciencia 3 → si no produces al tirar, `aqueduct:pick` da 1 recurso del banco). `CityCalendarPanel` marca la habilidad como "Activa".
- **(7) Comerciante (Mercader):** `state.merchant {ownerId, resource}`; +1 PV al dueño (`playerVP`) y 2:1 de ese recurso en el banco; se traspasa al jugarlo. Insignia en el marcador.
- **(4/5) Comercio de mercancías:** entre jugadores (`TradeOffer.giveCommodities/receiveCommodities`) y con banco/puertos (`trade:bank` con `giveKind/receiveKind`, ratio por `bankTradeRatioCK`). UI en `TradeModal`/`TradeIncomingModal`.
- **(2) Lobby:** en C&K la 2ª construcción se rotula "Ciudad de salida" (icono + copy).
- **(8) Layout:** en tablet/desktop, Calendario de ciudad, Defensa y Cartas de progreso pasan a la 3ª columna sobre los dados.

**Entregado (`docs/cambios-CC-plan-3.md`, julio 2026, rama `main`):** 3ª tanda de bugs C&K de `cambios.txt`, commit por cambio (89 tests de servidor + build de cliente):
- **B3 Acueducto en el 7:** el backend ya marcaba a los de Ciencia ≥3 (helper `aqueductBeneficiaries` + tests); el bug era de **z-index** del modal — `AqueductPickModal` subió a `z-[70]` (sobre el flujo del ladrón) para resolverse en un 7.
- **B4 Comercio mercancía↔recurso:** **ya funcionaba** (4:1 / 3:1 puerto / 2:1 Guilda); se añadieron tests confirmatorios. (Si se quiere otro ratio, es decisión de mesa.)
- **B2 Calendario:** se quitaron los labels "Habilidad" y "Arrebatar"; queda solo "Metrópolis" alineada bajo el nivel 4.
- **B6 Reciclaje:** las cartas de progreso ya no se acaban (`progressDiscards` + `drawProgressCard`).
...
**Entregado (`docs/cambios-CC-plan-5.md`, julio 2026, rama `main`):** 5ª tanda de bugs C&K de `cambios.txt`, commit por cambio (101 tests de servidor + build de cliente):
- **B4 Acueducto en el 7:** el Acueducto (Ciencia 3) ya **no** da recurso del banco en un 7 (`turn:rollCK` omite `pendingAqueductPick` cuando `production === 7`: el 7 bloquea la producción por el ladrón, no por falta de fichas).
- **B7 Saboteador automático:** cada oponente con PV **≥** los tuyos descarta la mitad de su mano (recursos + mercancías, floor) al banco (`saboteurDiscardCount` + `discardRandomMixed`).
- **B8 Boda automática:** cada oponente con **más** PV que tú te regala 2 cartas al azar (`stealRandomMixed`).
- **B6 Puerto de mercancías automático:** por cada oponente con mercancías, le das 1 recurso y recibes 1 mercancía (`commercialHarborExchange`).
- **B3 Obispo automático:** mueve el ladrón en "modo Obispo" (`pendingBishop` en estado/vista) y roba 1 carta a **cada** dueño del hex; en desierto/ficha vacía aplica `robberEmptyGivesResource`. Reutiliza `robber:move`/`robber:moveEmpty`; el selector del ladrón muestra un aviso de contexto.
- **B2 Grúa:** `CityCalendarPanel` refleja el descuento (`me.craneDiscount` → costo efectivo `max(0, costo-1)`), habilita "Mejorar" con el costo con descuento y muestra un chip "Grúa −1" con el costo tachado.
- **B1 Ícono de Ciencia:** se reescaló el **símbolo interior** del medallón (el compás verde era ~88 px de ancho vs ~156 px de las otras disciplinas); ahora empareja el peso visual de Comercio/Política. Marco de cobre intacto.
- **B5 Tratos vacíos/desiguales:** la funcionalidad **ya existía** (toggle `unequalTrades`, `trade:offer`/`trade:respond`, `TradeModal`/`TradeIncomingModal`); se añadieron tests confirmatorios y se mejoró la ayuda del toggle "Intercambios desiguales" para mencionar explícitamente los "tratos vacíos de un lado".
- **Cierre "revisa todas las cartas en mesa":** ahora el servidor automatiza **21 de 25** cartas de progreso; solo quedan **4 en mesa** por geometría física/tirada (`alchemist`, `inventor`, `diplomat`, `intrigue`). `PROGRESS_TABLE_RESOLVED` y los copys reflejan el cambio.

**Entregado (`docs/internal_prompts/cambios.txt`, julio 2026, rama `main`):** 6ª tanda — 1 bug del **juego base**, orquestado con el equipo de agentes (105 tests de servidor + build de cliente):
- **Puerto por poblado en el lobby (antes "Mis puertos"):** el lobby permitía auto-asignar puertos arbitrarios (`player:setPorts`) desligados de los poblados, dejando poblados con puerto y 3 fichas (imposible en Catán). Ahora el puerto se agrega **por construcción de salida** con el mismo patrón que la partida (`PortPickerSheet` extraído a `client/src/components/PortPickerSheet.tsx` para evitar import circular) y un poblado/ciudad con puerto queda **topado a 2 fichas** (UI en `InitialBuildSetup` + `validateBuildings`, que ya imponía `maxSpots = port ? 2 : 3`). **Los puertos son 100% derivados de los edificios:** `player:setBuildings` fija `player.ports` desde `building.port` (única fuente de verdad, en lobby y partida) y se **eliminó el handler `player:setPorts`** junto con el selector "Mis puertos" (`PortsModal`) del lobby. Pasada de QA: `PortPickerSheet` recupera la a11y de diálogo (focus-trap/ESC/`role=dialog`/`aria-labelledby`) que tenía el modal anterior, y ambos badges de puerto (lobby e in-game) llevan `aria-label` (⚓ marcado `aria-hidden`). Pipeline de agentes: ux-architect (brief) → backend (orquestador: `handlers.ts` + tests) + ui-engineer (frontend) → ux-writer (copy) + qa-auditor (a11y). El ux-architect determinó que visual-designer y motion-engineer no aplicaban (reuso de un patrón ya diseñado). Seguimiento no bloqueante (QA P2/P3, preexistentes/heredados): contraste del micro-label "editar", `pb-safe-bottom` no-op en el sheet, y unificar a `role="radiogroup"` el picker de puerto.

**Entregado (`docs/internal_prompts/cambios.txt` / instructivo, julio 2026, rama `main`):** pantalla **"Cómo jugar"** in-app, orquestada con TODO el equipo de agentes (build de cliente en verde; verificada renderizada en Chrome headless):
- **Feature:** instructivo completo en español accesible desde una card "Cómo jugar" en la HomeScreen (visible para invitado y logueado). 12 secciones (orden de una partida) en `CollapsibleSection` con índice/TOC de anchor-scroll (el índice **abre** la sección destino, modo controlado). La sección estrella **"Registrar tus poblados de salida"** es ilustrada con la estructura del PDF de referencia: **foto del tablero físico → conector → captura de la app → tarjeta "Resultado"**, en 5 bloques (3 fichas, puerto 1/2 fichas + tope, desierto, ficha compartida vs. nueva). Aviso de diseño: las capturas no siempre coinciden con su foto (sesión exploratoria) → los `alt` describen la imagen real y las leyendas enseñan el concepto genérico.
- **Navegación:** `homeView` extendido a `'instructions'` (store.ts + App.tsx, sin exigir cuenta). Nueva pantalla `client/src/screens/InstructionsScreen.tsx`; contenido/copy en `client/src/screens/instructionsContent.ts` (autoría ux-writer, 12 secciones + 5 bloques + 17 alt).
- **Imágenes:** 17 fotos del tablero/app en `client/public/instructions/` (las JPEG del tablero optimizadas de 19 MB → **1.26 MB** con Pillow, 828×1100). Lazy + `aspect-ratio` (cero CLS); las secciones colapsadas no montan hijos → no descargan imágenes. Originales sin optimizar en `docs/board_game_photos/`.
- **Pipeline de agentes:** ux-architect (IA/brief) → ux-writer (copy) → ui-engineer (build) → visual-designer (pulido) → motion-engineer (scroll-reveal, reduced-motion seguro) → qa-auditor (a11y/responsive/perf, 4/4, sin P0/P1). Backend intacto (feature 100% frontend).
- **Ampliación (mismo día):** las **11 secciones de texto** también se ilustran con **capturas reales de la app tomadas del walkthrough** (19 en total, 1–2 por sección), como `figures` sueltas con leyenda (campo `figures?: InstructionImage[]` en `InstructionSection`; render `SectionFigures` reusando el reveal e `IMAGE_DIMS`). Capturas curadas de `docs/screenshots/` → optimizadas a JPEG en `public/instructions/` (2.85 MB → 653 KB). El índice abre la sección destino (modo controlado de `CollapsibleSection`). Verificado en Chrome headless (CDP).

**Pendiente / futuro:** celebración de logros y XP en `WinnerScreen` al terminar la partida (hoy se ven en el perfil); y los ganchos de `docs/internal_prompts/pending-phase3.md` (foto del tablero, paired players, subida de avatar).
